...
SSAGES.set_backend(:HOOMD)
simulation = hoomd.context.current
system_view = SSAGES.wrap(simulation)
...
sampler = SSAGES.ABF(grid, ξ, 100)(system_view)

hook = SSAGES.bind(simulation, sampler)
...
scatter(θ, (dA .- reverse(dA)) ./ 2; label = "")
scatter!(θ, dA; label = "")
